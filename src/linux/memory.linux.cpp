...
ox::result<ox::memory> ox::mem::alloc_as_memory(bytes size, bytes alignment) {
...
ox::result<ox::pointer<void>> ox::mem::alloc(bytes size, bytes alignment) {
    OX_ASSERT(size != bytes(0));
    OX_ASSERT(alignment != bytes(0));
    if (size == bytes(0) || alignment == bytes(0)) return err::memory_size_is_zero;
...
ox::result_t ox::mem::free(vptr &ptr) {
    OX_ASSERT(ptr != nullptr);
    if (!ptr) return err::already_freed;
    ::free(ptr.get());
    ptr = nullptr;
    return ok;
}
...
ox::result_t ox::mem::free(memory &mem, bytes alignment){
    ...
    if (alignment != no_alignment) {
        free_aligned_sized(mem.pointer.get(),alignment.ct,mem.size.ct);
    }else {
        free_sized(mem.pointer.get(),mem.size.ct);
    }
    mem.pointer = nullptr;
    mem.size = bytes(0);
    return ok;
...
ox::result<ox::memory> ox::mem::valloc_as_memory(bytes size) {
    OX_ASSERT(size != bytes(0));
...
ox::result<ox::pointer<void>> ox::mem::valloc(bytes size) {
    OX_ASSERT(size != bytes(0));
    if (size == bytes(0)) return err::memory_size_is_zero;
...
ox::result_t       ox::mem::vfree(memory& o) {
    ...
    ::munmap(o.pointer.get(),o.size.ct);
    o.pointer = nullptr;
    o.size = bytes(0);
    return ok;
}

ox::result_t ox::mem::vfree(vptr &o,bytes size) {
    OX_ASSERT(o != nullptr);
    if (!o) return err::already_freed;
    if (size == bytes(0)) return err::memory_size_is_zero;

    ::munmap(o.get(),size.ct);
    o = nullptr;
    return ok;
}